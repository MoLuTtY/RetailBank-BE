...
public interface AccountFeignClient {
	...
	Account getCustomerSavingsAccount(@PathVariable Long customerId);
	
	@GetMapping("/api/accounts/find-current-accounts/{customerId}")
	Account findCurrentAccounts(@PathVariable Long customerId);
	
	@GetMapping("/api/accounts/viewCustomer-by-accountNo-and-accountType/{accountNo}/{accountType}")
	Account viewCustomerByAccountNoAndAccountType(@PathVariable Long accountNo, @PathVariable AccountType accountType);
	
	@DeleteMapping("/api/accounts/delete-account/{accountNo}")
    void deleteAllAccountsByAccountNo(@PathVariable Long accountNo);
}
